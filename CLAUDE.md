...
6. **Every entry point to the packing flow must reset all packing state.** The menu "Pack a Box"
   handler and `nextBox()` both must clear `S.desc`, `S.dest`, `S.tier`, `S.photos`, `S.items`,
   and all related fields. Failure to do this causes stale data from the previous box to leak
   into the next box's record. (Bug found and fixed in v18b.)
