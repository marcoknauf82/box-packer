...
- **Session state lost on browser refresh** — by design at this stage (pre-backend); all inventory is in Google Sheet
...
- Offline queue for failed sheet writes (service worker background sync)
